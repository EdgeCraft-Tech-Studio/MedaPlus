...
# Same fix pattern as user.py:
#   - Meta.indexes (all 6) removed.
#   - Meta.constraints (the partial UniqueConstraint on user+device_id where
#     is_revoked=False) removed — no clean Mongo equivalent, and partial
#     constraints aren't what you want anyway with indexes off.
#   - db_index=True stripped from every field, including id, user (FK),
#     device_id, device_type, ip_address, is_revoked, expires_at, created_at.
#   - Kept unique=True on session_token_hash and refresh_token_hash only —
#     those are the actual "duplicate index" culprits in the original (they
#     had unique=True *and* db_index=True on the same field, which is a
#     redundant double index on one column). Removing db_index=True there
#     keeps a single, non-duplicated unique index on each, since token-hash
#     lookups genuinely need to stay fast and unique.
# Logic (properties, methods, classmethods, querysets) is untouched.
...
class UserSession(models.Model):
    ...
    class Meta:
        ...
        # NOTE: the old partial UniqueConstraint (one active session per
        # user+device_id) enforced "revoke old session before a new one for
        # the same device" at the DB level. Without it, that's only true
        # because your code always calls revoke_for_device() before creating
        # a new session (see below) — nothing stops a race from creating two
        # active rows for the same device if that call is skipped somewhere.
