...
MIDDLEWARE = [
    ...
    # CHANGED: added WhiteNoise right after SecurityMiddleware. This lets
    # Django/gunicorn serve your collected static files itself, so you
    # don't need a separate nginx container just for the backend's static
    # files. Requires `pip install whitenoise` (added to requirements.txt).
...
CACHES = {
    "default": {
        ...
        # CHANGED: was hardcoded to "redis://127.0.0.1:6379/1". Inside
        # docker-compose, redis is not on 127.0.0.1 — it's a separate
        # container reachable by its service name "redis". This now reads
        # REDIS_HOST from the environment (set to "redis" by docker-compose,
        # falls back to 127.0.0.1 so it still works if you run Django
        # directly on your PC without Docker).
